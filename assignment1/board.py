...
class GoBoard(object):
    ...
    def _diag_win(self, point, color):
        # Check win for rows and columns
        diag_neighbors = self._diag_neighbors(point)
        top_left_counter = 1  # for \
        top_right_counter = 1  # for /
        for nb in diag_neighbors:
            diff = nb-point
            counter = 0
            while self.board[nb] == color:
                counter += 1
                nb += diff
            if abs(diff) == (self.NS+1):
                top_left_counter += counter
                if top_left_counter >= 5:
                    return True
            else:
                top_right_counter += counter
                if top_right_counter >= 5:
                    return True
        return False
